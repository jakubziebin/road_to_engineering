...
from download_and_prepare_datas.preparing_datas_to_send import get_price_of_electricity
from download_and_prepare_datas.download_energy_prices import PricesOfEnergy
...
def write_real_value(node_id, value):
    client_node = client.get_node(node_id)
...
def write_bool_value(node_id, value):
    client_node = client.get_node(node_id)
    client_node_value = value
...
if __name__ == "__main__":
    ...
    eval(f"write_real_value('ns=3;s='OpcuaPythonTransfer'.{VARIABLES_IN_PLC[0]}, {electricity_price})")
